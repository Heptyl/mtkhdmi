...
#define LOG_TAG "HdmirxCamOfflnSsn@3.6"
...
#include "HdmirxCameraOfflineSession.h"
...
// static instance
HandleImporter HdmirxCameraOfflineSession::sHandleImporter;

using V3_5::implementation::HdmirxCameraDeviceSession;

HdmirxCameraOfflineSession::HdmirxCameraOfflineSession(
...
bool HdmirxCameraOfflineSession::initialize() {
...
void HdmirxCameraOfflineSession::initOutputThread() {
    if (mOutputThread != nullptr) {
        ALOGE("%s: OutputThread already exist!", __FUNCTION__);
        return;
    }

    mBufferRequestThread = new HdmirxCameraDeviceSession::BufferRequestThread(
            this, mCallback);
...
bool HdmirxCameraOfflineSession::OutputThread::threadLoop() {
...
Status HdmirxCameraOfflineSession::importBuffer(int32_t streamId,
...
status_t HdmirxCameraOfflineSession::fillCaptureResult(
...
Status HdmirxCameraOfflineSession::processCaptureResult(std::shared_ptr<HalRequest>& req) {
    ATRACE_CALL();
    // Fill output buffers
...
void HdmirxCameraOfflineSession::invokeProcessCaptureResultCallback(
...
Status HdmirxCameraOfflineSession::processCaptureRequestError(
...
ssize_t HdmirxCameraOfflineSession::getJpegBufferSize(
        uint32_t /*width*/, uint32_t /*height*/) const {
    // Empty implementation here as the jpeg buffer size is passed in by ctor
    return 0;
};

void HdmirxCameraOfflineSession::notifyError(uint32_t frameNumber, int32_t streamId, ErrorCode ec) {
    NotifyMsg msg;
...
Return<void> HdmirxCameraOfflineSession::setCallback(const sp<ICameraDeviceCallback>& cb) {
...
Return<void> HdmirxCameraOfflineSession::getCaptureResultMetadataQueue(
...
void HdmirxCameraOfflineSession::cleanupBuffersLocked(int id) {
...
Return<void> HdmirxCameraOfflineSession::close() {
    Mutex::Autolock _il(mInterfaceLock);
    {
