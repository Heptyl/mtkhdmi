...
class SectionHelper {
 protected:
  const std::vector<VendorTagSection_t>& mvSection;
  size_t mTagCount;

 public:
  SectionHelper() : mvSection(getGlobalSections()), mTagCount(0) {
    mTagCount = 0;
  ...
  auto getTagCount() const -> size_t { return mTagCount; }

  auto getSection(uint32_t tag) const -> VendorTagSection_t const* {
    for (auto const& s : mvSection) {
      if (s.tagStart <= tag && s.tagEnd > tag) {
        ALOGV("getSection: tag:0x%x in section:%s", tag, s.sectionName);
        return &s;
      }
    }
    ALOGE("not a vender tag:0x%x", tag);
    return nullptr;
  }
  ...
  auto getTag(uint32_t tag) const -> VendorTag_t const* {
    if (auto s = getSection(tag)) {
      ...
      if (t != s->tags.end()) {
        ALOGV("tag:0x%x %s", tag, t->second.tagName);
        return &(t->second);
      }
      ALOGE("tag:0x%x not in section:%s", tag, s->sectionName);
    }
    return nullptr;
  }
...
static auto static_get_all_tags(const vendor_tag_ops_t* /*ops*/,
                                uint32_t* tag_array) -> void {
  if (!tag_array) {
    ALOGE("bad tag_array");
    return;
  }
...
static auto static_get_section_name(const vendor_tag_ops_t* /*ops*/,
                                    uint32_t tag) -> const char* {
  return getSectionHelper().getSectionName(tag);
}

static auto static_get_tag_name(const vendor_tag_ops_t* /*ops*/, uint32_t tag)
    -> const char* {
  return getSectionHelper().getTagName(tag);
}

static auto static_get_tag_type(const vendor_tag_ops_t* /*ops*/, uint32_t tag)
    -> int {
...
static int setAsGlobalVendorTagSections(vendor_tag_ops_t const* ops) {
  ...
  if (!pLib) {
    ...
    ALOGE("dlopen: %s error=%s", szModulePath,
  ...
  if (!pfnEntry) {
    char const* err_str = ::dlerror();
    ALOGE("dlsym: %s error=%s", szEntrySymbol,
              (err_str ? err_str : "unknown"));
    goto lbExit;
  }
  //
  typedef int (*set_camera_metadata_vendor_ops)(
      const vendor_tag_ops_t* query_ops);
...
lbExit:
  ...
  ALOGD("- %p:%s(%p) return res:%s(%d) in %s", pfnEntry, szEntrySymbol, ops,
            strerror(-status), status, szModulePath);
  ALOGE_IF(0 != status, "static vendor metadata may fail");
  return status;
}
 ...
 *
 ******************************************************************************/
class VendorTagDescriptorImpl : public NSCam::IVendorTagDescriptor {
 public:
