 ...
 */
#define LOG_TAG "HdmirxCamUtils@3.4"
//#define LOG_NDEBUG 0
#include <log/log.h>
...
status_t fillHdmiRxInfo(common::V1_0::helper::CameraMetadata &md, int32_t width, int32_t height,
	int32_t hdmi_mode, int32_t framerate, int32_t hdcp, int32_t interlace) {

	const int32_t hdmirx_info [] = {
		width,
		height,
		hdmi_mode,
		framerate,
	...
	UPDATE(md, MTK_CAMERA_HDMI_RX_INFO, hdmirx_info, ARRAY_SIZE(hdmirx_info));
...
const char* HdmirxCameraConfig::kDefaultCfgPath = "/vendor/etc/hdmirx_camera_config.xml";


HdmirxCameraConfig HdmirxCameraConfig::loadFromCfg(const char* cfgPath) {
    using namespace tinyxml2;
    HdmirxCameraConfig ret;

    XMLDocument configXml;
    XMLError err = configXml.LoadFile(cfgPath);
    if (err != XML_SUCCESS) {
        ALOGE("%s: Unable to load Hdmirx camera config file '%s'. Error: %s",
                __FUNCTION__, cfgPath, XMLDocument::ErrorIDToName(err));
        return ret;
    } else {
        ALOGI("%s: load Hdmirx camera config succeed!", __FUNCTION__);
    }

    XMLElement *extCam = configXml.FirstChildElement("HdmirxCamera");
    if (extCam == nullptr) {
        ALOGI("%s: no Hdmirx camera config specified", __FUNCTION__);
        return ret;
    }

    XMLElement *providerCfg = extCam->FirstChildElement("Provider");
    if (providerCfg == nullptr) {
        ALOGI("%s: no Hdmirx camera provider config specified", __FUNCTION__);
        return ret;
    }

    XMLElement *cameraIdOffset = providerCfg->FirstChildElement("CameraIdOffset");
    if (cameraIdOffset != nullptr) {
        const char* text = cameraIdOffset->GetText();
        if (text != nullptr) {
            ret.cameraIdOffset = std::atoi(text);
            ALOGD("%s, CameraIdOffset = %d", __FUNCTION__, ret.cameraIdOffset);
        }
    }
    ...
    while (id != nullptr) {
        ...
        if (text != nullptr) {
            ...
            ALOGI("%s: device %s will be ignored by Hdmirx camera provider",
                    __FUNCTION__, text);
        }
    ...
    if (deviceCfg == nullptr) {
        ALOGI("%s: no Hdmirx camera device config specified", __FUNCTION__);
        return ret;
    }
    ...
    ALOGI("%s: Hdmirx camera cfg loaded: maxJpgBufSize %d,"
...
bool HdmirxCameraConfig::updateFpsList(tinyxml2::XMLElement* fpsList,
...
HdmirxCameraConfig::HdmirxCameraConfig() :
    ...
    fpsLimits.push_back({/*Size*/{ 320,  240}, /*FPS upper bound*/60.0});
    fpsLimits.push_back({/*Size*/{ 640,  480}, /*FPS upper bound*/60.0});
    fpsLimits.push_back({/*Size*/{1280,  720}, /*FPS upper bound*/60.0});
    fpsLimits.push_back({/*Size*/{1920, 1080}, /*FPS upper bound*/30.0});
    fpsLimits.push_back({/*Size*/{3840, 2160}, /*FPS upper bound*/30.0});
    minStreamSize = {0, 0};
    maxVideoSize = {0, 0};
}
