...
#ifndef ANDROID_HARDWARE_CAMERA_DEVICE_V3_4_HDMIRXCAMUTIL_H
#define ANDROID_HARDWARE_CAMERA_DEVICE_V3_4_HDMIRXCAMUTIL_H
...
struct HdmirxCameraConfig {
    static const char* kDefaultCfgPath;
    static HdmirxCameraConfig loadFromCfg(const char* cfgPath = kDefaultCfgPath);

    // CameraId base offset for numerical representation
    uint32_t cameraIdOffset;

    // List of internal V4L2 video nodes Hdmirx camera HAL must ignore.
...
// Aspect ratio is defined as width/height here and HdmirxCameraDevice
...
struct HalRequest {
    ...
    uint32_t buf_num;
    uint32_t buf_index;
	int32_t hdmirx_width;
	int32_t hdmirx_height;
	int32_t hdmirx_framerate;
	int32_t hdmirx_dvi;  //  0: DVI, 1: not DVI
	int32_t hdmirx_hdcp; // 1: decrypted, 0: decrypt fail or not encrypted
	int32_t hdmirx_interlace;
};
...
status_t fillHdmiRxInfo(common::V1_0::helper::CameraMetadata& md, int32_t width, int32_t height,
	    int32_t hdcp_mode, int32_t framerate, int32_t hdcp, int32_t interlace);
...
struct OutputThreadInterface : public virtual RefBase {
    ...
    virtual void setGrallocBufferStatus(int index) = 0;
};
