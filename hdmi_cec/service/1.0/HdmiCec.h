    ...
    static void eventCallback(const hdmi_event_t* event, void* /* arg */) {
        if (mCallback != nullptr && event != nullptr) {
            if (event->type == HDMI_EVENT_CEC_MESSAGE) {
                ...
                auto ret = mCallback->onCecMessage(cecMessage);
                if (!ret.isOk()){
                    LOG(ERROR) << "eventCallback  onCecMessage() failed";
                }
                ...
                auto ret = mCallback->onHotplugEvent(hotplugEvent);
                if (!ret.isOk()){
                    LOG(ERROR) << "eventCallback  onHotplugEvent() failed";
                }
            }
        }
    }
